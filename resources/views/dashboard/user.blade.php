                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-3 col-md-6 p-1">
                            <a href="/user?status=active" onclick="showLoader()"
                                class="py-3 form-control btn btn-{{ $status == 'active' ? 'info' : 'secondary' }}">Active</a>

                        </div>
                        <div class="col-lg-3 col-md-6 p-1">
                            <a href="/user?status=inactive" onclick="showLoader();"
                                class="py-3 form-control btn btn-{{ $status == 'inactive' ? 'info' : 'secondary' }}">Inactive</a>
                        </div>
                    </div>
                </div>
                @can('create_user')
                    <div class="col-lg-2 col-md-12 text-end p-1">
                        <button class="form-control btn btn-primary" onclick="modalAdd()">
                        ...
                        @foreach ($user as $u)
                            <tr>
                                <td class="ps-5">
                                    <div class="form-check form-switch">
                                        ...
                                        @else
                                            @can('delete_user')
                                                <input class="form-check-input" type="checkbox" id="flexSwitchCheckChecked"
                                                    checked onclick="inactive({{ $u->id }}, this)">
                                            @endcan
                                            <b><i><label class="form-check-label ms-0"
                                ...
                                <td><a href="javascript:void(0)"
                                        onclick="modalUser({{ $u->id }})">{{ $u->name }} <i
                                    ...
                                    <br><small class="mt-1">Email : {{ $u->email }}</small>
                                </td>
                                <td>{{ $u->division }}</td>
                                <td>{{ $u->company }}</td>
